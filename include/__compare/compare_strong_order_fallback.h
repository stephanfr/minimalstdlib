    ...
    namespace __compare_strong_order_fallback
    {
        struct __fn
        {
            ...
            _MINIMAL_STD_HIDDEN static constexpr auto __go(_Tp &&__t, _Up &&__u, __priority_tag<1>) noexcept(
                noexcept(strong_order(forward<_Tp>(__t), forward<_Up>(__u))))
                -> decltype(strong_order(forward<_Tp>(__t), forward<_Up>(__u)))
            {
                return strong_order(forward<_Tp>(__t), forward<_Up>(__u));
            }
            ...
            _MINIMAL_STD_HIDDEN static constexpr strong_ordering __go(_Tp &&__t, _Up &&__u, __priority_tag<0>) noexcept(
                noexcept(forward<_Tp>(__t) == forward<_Up>(__u)  ? strong_ordering::equal
                         : forward<_Tp>(__t) < forward<_Up>(__u) ? strong_ordering::less
                                                                           : strong_ordering::greater))
            {
                return forward<_Tp>(__t) == forward<_Up>(__u) ? strong_ordering::equal
