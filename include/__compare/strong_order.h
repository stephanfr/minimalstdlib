    ...
    namespace __strong_order
    {
        ...
        struct __fn
        {
            ...
            _MINIMAL_STD_HIDDEN static constexpr auto __go(_Tp &&__t, _Up &&__u, __priority_tag<2>) noexcept(
                noexcept(strong_ordering(strong_order(forward<_Tp>(__t), forward<_Up>(__u)))))
                -> decltype(strong_ordering(strong_order(forward<_Tp>(__t), forward<_Up>(__u))))
            {
                return strong_ordering(strong_order(forward<_Tp>(__t), forward<_Up>(__u)));
            }
            ...
            _MINIMAL_STD_HIDDEN static constexpr strong_ordering __go(_Tp &&__t, _Up &&__u, __priority_tag<1>) noexcept
            {
                ...
                else if (__t == __u)
                {
                    ...
                    else
                    {
                        ...
                        {
                            int __texp, __uexp;
                            (void)frexp(__t, &__texp);
                            (void)frexp(__u, &__uexp);
                ...
                else
                {
                    ...
                    {
                        static_assert(sizeof(_Dp) == 0, "strong_order is unimplemented for this floating-point type");
                    }
                    else if (__t_is_nan && __u_is_nan)
                    {
                        ...
                        {
                            return bit_cast<_IntType>(__t) <=> bit_cast<_IntType>(__u);
                        }
                    }
                    else if (__t_is_nan)
                    {
            ...
            _MINIMAL_STD_HIDDEN static constexpr auto __go(_Tp &&__t, _Up &&__u, __priority_tag<0>) noexcept(
                noexcept(strong_ordering(compare_three_way()(forward<_Tp>(__t), forward<_Up>(__u)))))
                -> decltype(strong_ordering(compare_three_way()(forward<_Tp>(__t), forward<_Up>(__u))))
            {
                return strong_ordering(compare_three_way()(forward<_Tp>(__t), forward<_Up>(__u)));
            }
