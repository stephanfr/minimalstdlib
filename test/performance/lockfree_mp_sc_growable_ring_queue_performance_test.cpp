...
namespace
{
                                         ...
                                         size_t max_capacity = static_cast<size_t>(-1))
    {
        ...
        for (int i = 0; i < rounds; ++i)
        {
            ...
            new_delete_test_resource resource;
            typename ring_perf_runner<SegmentCount, NumProducers, NumConsumers>::alloc_t alloc(&resource);
            typename ring_perf_runner<SegmentCount, NumProducers, NumConsumers>::queue_t q(
                alloc, initial_capacity, 5, 4, 0, max_capacity);
            auto [ops, c] = r.run(q, total_items);
            if (ops > best_ops)
            {
                best_ops = ops;
            }
